...
for (let i=0; i < arrayTest3.length; ++i){
    practiceArray.push(reverseCharacters(arrayTest3[i]));
}
...
function funPhrase(str) {
    let tinyStr = "";
    if (str.length <= 3) {
        tinyStr = str.slice(str.length-1);
    } else if (str.length > 3) {
        tinyStr = str.slice(0,3);
    }
    return (`We put the ${tinyStr} in ${str}`);
}


console.log(funPhrase("fir"));
